...
实验解答保存在 [result.txt](../labs/attack/) 中；这里运行 `./ctarget -q` 要用 `-q` ，毕竟不是 CMU 的学生，`-q` 的作用: `Don’t send results to the grading server` 。  
...
在解题之前，和 bomb lab 一样， 首先反汇编可执行程序，生成汇编代码。  
 `objdump -d ctarget > ctarget_disas.txt`   
 利用汇编代码来分析程序。
...
主要思路是找到touch1的起始地址的字节表示的位置，使得getbuf结尾处的ret指令会控制转移到touch1

题目要求输入字符串，攻击 `getbuf` ，在 `test` 函数中调用 `touch1` 函数；       
第一题 `getbuf` 中 ：

```
000000000040169d <getbuf>:
  40169d:	48 83 ec 28          	sub    $0x28,%rsp
  4016a1:	48 89 e7             	mov    %rsp,%rdi
  4016a4:	e8 87 02 00 00       	callq  401930 <Gets>
```

 栈有 `0x28` 即 40 个字节，注意 x86-64 在函数调用时会自动将返回地址压入栈，因此调用函数时栈顶就是返回地址，只要修改它就能调用其他的函数了，输入 40 个字节后，使栈溢出，再输入个地址就能破坏 `getbuf` 返回地址，返回时就会调用修改了的地址对应的函数，这里的是个八字节的 64 位地址，按题目要求就是 `touch1` 函数的地址， `00000000004016b3 <touch1>:` ，注意这里机器要用小端法存入地址写成： `b3 16 40 00 00 00 00 00` ，因此第一题答案就是任意 40 个字节加上该地址。   
答案可以是：
...
![](https://cdn.jsdelivr.net/gh/growvv/image-bed/images/20200318145254.png)
...
这次 lab 较简单，涉及了代码注入、栈随机化，ROP 攻击等内容，通过这个 lab ，我对汇编、栈都有了更深的了解，以后在写代码时也会更加注重这些点。