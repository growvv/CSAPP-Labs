...
上段代码将存储了地址的 `%rdi` 和 `%rsi` 的值先后传到了 `string_length` 中， `%rdi` 即为原函数的第一个参数，即 `input` ，是输入的 phase 的首地址， `%rsi` 为设定的地址，是设定的 phase 的地址；上面的函数判断两个 phase 是否相同，不相同就 bomb! 因而将这个设定的 phase 打印出来就是结果了。
